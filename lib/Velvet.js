...
class VelvetError extends ExtendableError {
  constructor(message) {
    super(message);
  }
}
...
class Velvet {
  ...
  initCoreModules() {
    ...
    _.each(__Modules, function(__Module, key) {
      ...
      let apiRouter = m.apiRouter(new Router());
      self.koa.use(adminRouter.routes());
      self.koa.use(apiRouter.routes());
    });
  ...
  setupAuthentication() {
    ...
    /**
     * Setup the template language used by Velvet.
  ...
  setupTemplates(engine) {
    ...
    engine = engine || 'swig';
    if (engine !== 'swig')
      throw new VelvetError('Swig is the only supported engine right now.');
...
Velvet.VelvetModule = require('./VelvetModule.js');
Velvet.Error = VelvetError;
exports.Velvet = module.exports = Velvet;
